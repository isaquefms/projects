...
public class SuporteTecnico {
	...
	public SuporteTecnico() {
		bancoSolucao = new HashMap<String, String>();
	...
	// Metodo para buscar uma solucao padrão.
	// O metodo precisa de um aprimoramento.
	public String buscaSolucao(String problema) {
		// Guardando todas as chaves em uma lista.
		Set<String> chaves = bancoSolucao.keySet();
		// Pegando cada chave da minha lista.
		for(String chave : chaves) {
			// Se a chave for igula a palavra problema.
			if(chave.equals(problema)) {
				// Retornamos a resposta da chave.
				return bancoSolucao.get(chave);
			}
		}
		// Caso não encontre retornamos uma mensagem default.
		return "Falha";
	}
	
	// Metodo para retornar uma resposta aleatoria.
	public String respostaDefault() {
		// Instanciando um gerador de numeros randomicos.
		Random gerador = new Random();
		// i ira armazenar um numero aleatorio de 0 a numero de elementos da lista - 1.
		int i = gerador.nextInt(respostaPadrao.size());
		// Retornamos a respota referente ao indice.
